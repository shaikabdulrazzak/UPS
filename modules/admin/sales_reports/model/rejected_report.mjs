...
// Model for Rejected Report
export default class RejectedReport {
	...
	/**
	 * Function to get rejected orders report list
	 *
	...
	async rejectedOrder(req,res,next){
		try{
			...
			/** Common conditions  */
			let commonConditions = { order_status : {$in : [Constants.ORDER_REJECTED,Constants.ORDER_REJECTED_BY_ADMIN]}};
			...
			if(isPost(req)){
				...
				let dbRes = await collection.aggregate([
					...
					{$facet : {
						list : [
									...
									{$match : {
										$expr: {
											$and : [
												{$eq: ["$order_id", "$$orderId"]},
												{$or: [
													{$eq: ["$status", Constants.ORDER_REJECTED]},
													{$eq: ["$status", Constants.ORDER_REJECTED_BY_ADMIN]},
												]}
											]
										},
									}},
							...
							{$project : {
								_id:1,unique_order_id:1,restaurant_name:1,order_price:1,rejection_reason:1,
				...
				asyncParallel({
					...
					total_rejected_orders : (callback)=>{
						/** Get total rejected orders **/
						collection.countDocuments(commonConditions).then(totalRejectedResult=>{
							callback(null, totalRejectedResult);
						}).catch(totalRejectedErr=>{
							callback(totalRejectedErr, 0);
						});
					}
				},(err,response)=>{
					...
					res.render('rejected_report',{
						...
						total_rejected_orders : response?.total_rejected_orders || 0,
					});
				});
			}
		}catch(err){
