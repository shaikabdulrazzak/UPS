        ...
        dataArray.map(function(menu,index) {
            ...
            if(typeof menu.childs == typeof undefined || menu.childs.length == 0 && !( level == 0 && index ==0 )){
                if (typeof menu.slug !== typeof undefined && menu.slug != "" && typeof permissions[menu.slug] !== typeof undefined ) {
                    ...
                    let only1SubChild = (Object.keys(lastChilds).length == 1) ? true : false;

                    html += '<ul>';
                    liExpendSpan = '<span title="Expand this branch" class="font-14 collapsed expend-btn '+(only1SubChild ? "hideMe" : "")+' "></span>';;

                    console.log('menu.slug');
                    console.log(menu.slug);

                    console.log('only1SubChild');
                    console.log(only1SubChild);
